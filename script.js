(function () {
  ...
  function wireGalleryAndLightbox() {
    ...
    var clips = cards.map(function (card) {
      var video = card.querySelector("video");
      return { src: video.getAttribute("src"), label: video.getAttribute("aria-label") };
    });
    ...
    function openAt(index) {
      ...
      var item = clips[activeIndex];
      lightboxVideo.setAttribute("src", item.src);
      lightboxVideo.setAttribute("aria-label", item.label);
      lightboxCount.textContent = (activeIndex + 1) + " / " + clips.length;
